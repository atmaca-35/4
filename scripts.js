document.addEventListener('DOMContentLoaded', async () => {
    ...
    function highlightWords(text) {
        ...
        // İşaretleyici etiketlere sahip metni işler
        for (const [key, value] of Object.entries(specialWords)) {
            const regex = new RegExp(`\\[SPECIAL:${key}\\]`, 'g');
            markedText = markedText.replace(regex, `<span class="highlight">${value}</span>`);
        }
    ...
    searchBox.addEventListener('input', (e) => {
        ...
        const firstMatchingWord = Object.keys(dictionaryData)
            .find(word => word.toLowerCase().startsWith(query.toLowerCase()));

        if (firstMatchingWord && query.length > 0) {
            ghostText.textContent = firstMatchingWord;
        } else {
            ghostText.textContent = "";
        }
    });

    // JavaScript Adjustment for font-size on resize
    window.addEventListener('resize', () => {
        if (window.innerWidth > 768) {
            wordCountElement.style.fontSize = '1rem'; 
        } else {
            wordCountElement.style.fontSize = '1rem';
        }
    });
